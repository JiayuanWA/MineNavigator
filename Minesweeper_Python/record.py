...
class MyAI(AI):
    ...
    def getAction(self, number: int) -> "Action":
        ...
        while True:
            ...
            # If there are no squares to probe, and the search didn't find any
    ...
    def search(self):
        ...
        mines_left = self.totalMines
        squares_left = len(squares)

        print("Squares:", squares)
        print("Mines left:", mines_left)
        print("Squares left:", squares_left)

        def backtrack(comb):
            print("Current combination:", comb)
            if len(comb) > squares_left:
                print("Combination too long, returning")
                return
            elif sum(comb) > mines_left:
                print("Too many mines in combination, returning")
                return
            else:
                for choice in [0, 1]:
                    comb.append(choice)
                    if sum(comb) == mines_left and len(comb) == squares_left:
                        ...
                        print("Combination", comb, "is", "valid" if valid else "invalid")
                        if valid:
                            # Only keep valid solutions
                            solutions.append(comb.copy())
                    backtrack(comb)
                    comb.pop()
                    print("Backtracked to combination:", comb)
        ...
        if solutions:
            ...
            for s in range(len(solutions)):
                for sq in range(len(solutions[s])):
                    ...
                    if current_square not in square_solution_counts:
                        ...
                        square_solution_counts[current_square] += solutions[s][sq]
            print("Square solution counts:", square_solution_counts)
            ...
            for square, count in square_solution_counts.items():
                if count == 0:
                    ...
                    self.squares_to_probe.append(square)
                    print("Added safe square:", square)
            ...
            if not added_safe_squares:
                ...
                print("Random solution chosen:", comb)
                for square, value in zip(squares, comb):
                    if value == 0:
                        # Currently just adding all squares marked as safe in the first solution in list
                        self.squares_to_probe.append(square)
                        print("Added safe square from random solution:", square)
        else:
            # No solutions, so pick a random square
            squares_left = list(set(self.board.keys()) - self.marked_squares - self.probed_squares - self.mines_flagged)
            ...
            print("No solutions found, picked random square:", next_square)
    ...
    def check_solution_validity(self, squares, comb):
        ...
        all_valid = True
        for square, value in zip(squares, comb):
            if not self.meets_constraints(square, value):
                all_valid = False
                break
    ...
    def meets_constraints(self, variable, val):
        ...
        print(f"Setting square ({x}, {y}) to {'mine' if val == 1 else 'safe'}")
        ...
        for n in neighbors:
            ...
            # Print the state of the neighbor being checked
            print(f"Checking neighbor square ({nx}, {ny}) - original constant: {neighbor_constant}, value: {neighbor_square.val}")

            # Skip if neighbor_square is still uncovered
            if neighbor_constant is not None:
                ...
                if neighbor_square.val is not None and neighbor_square.val != 1:
                    ...
                    print(f"Neighbor ({nx}, {ny}) - Mines: {mines}, Safe: {safe}, Unknown: {unknown}, Constant: {neighbor_constant}")
                    print("checks to see if it violates constraints")

                    if mines > neighbor_constant:
                        # Violation: too many mines
                        print(f"Violation at ({nx}, {ny}): too many mines (mines: {mines}, constant: {neighbor_constant})")
                        return False
                    elif (neighbor_constant - mines) > unknown:
                        # Violation: not enough mines
                        print(f"Violation at ({nx}, {ny}): not enough mines (mines: {mines}, unknown: {unknown}, constant: {neighbor_constant})")
                        return False

        print(f"Square ({x}, {y}) set to {'mine' if val == 1 else 'safe'} passes all constraints")
        return True
    ...
    def get_neighbor_count(self, variable):
        ...
        for nb in neighbors:
            ...
            if nbor_square.val == 1:
                ...
                safe_count += 1
            elif nbor_square.original_constant is None:
                unknown_count += 1
